...
const cadastrarPedido = async (req, res) => {
  try {
    ...
    const user = await user.findOne({ codigo: req.user.codigo })
    if (!user) {
    ...
    res.status(201).json({
      ...
      pedido: await novoPedido.populate([
        { path: 'userId', select: 'codigo nome email perfil' },
...
const listarPedidos = async (req, res) => {
  try {
    ...
    const pedidos = await Pedido.find({ userId: user._id })
      .populate('userId', 'codigo nome email perfil')
...
const historicoPedidos = async (req, res) => {
  try {
    ...
    const historico = await Pedido.find({ userId: user._id })
      .populate('userId', 'codigo nome email perfil')
      .populate('itens.produtoId', 'codigo nome preco status')
      .sort({ data: -1 })
...
// 🗑️ Excluir todos os pedidos de um user (admin)
const excluirPedidosuserAdmin = async (req, res) => {
  try {
    const { codigouser } = req.params
    const user = await user.findOne({ codigo: Number(codigouser) })
    if (!user) {
      return res.status(404).json({ mensagem: 'user não encontrado' })
    }
    const resultado = await Pedido.deleteMany({ userId: user._id })
    res.status(200).json({ mensagem: 'Pedidos do user excluídos com sucesso', resultado })
  } catch (err) {
    console.error('❌ Erro ao excluir pedidos do user:', err)
    res.status(500).json({ mensagem: 'Erro ao excluir pedidos do user', erro: err.message })
  }
}
